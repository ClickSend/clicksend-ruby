...
module ClickSendClient
  class FaxApi
    ...
    # @option opts [Integer] :limit Number of records per page (default to 10)
    # @return [String]
    def fax_receipts_get(q, opts = {})
      data, _status_code, _headers = fax_receipts_get_with_http_info(q, opts)
      data
    end
    ...
    # @option opts [Integer] :limit Number of records per page
    # @return [Array<(String, Fixnum, Hash)>] String data, response status code and response headers
    def fax_receipts_get_with_http_info(q, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: FaxApi.fax_receipts_get ...'
      end
      # verify the required parameter 'q' is set
      if @api_client.config.client_side_validation && q.nil?
        fail ArgumentError, "Missing the required parameter 'q' when calling FaxApi.fax_receipts_get"
      end
      if @api_client.config.client_side_validation && !opts[:'page'].nil? && opts[:'page'] < 1
